...
formNode.prototype.resetValues = function () {
  ...
  if (this.view && this.view.inputfield) {
    ...
    _.each(params, function (param) {
      ...
      $('[name="' + escapeSelector(param.name) + '"]', $(instance.el)).val('');
    });
  }
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  if (this.formElement) {
    ...
    _.each([
      ...
      if (_.isString(instance.formElement[prop])) {
        if (instance.formElement[prop].indexOf('{{values.') !== -1) {
          // This label wants to use the value of another input field.
          // Convert that construct into {{jsonform.getValue(key)}} for
          // Underscore to call the appropriate function of formData
          // when template gets called (note calling a function is not
          // exactly Mustache-friendly but is supported by Underscore).
          instance[prop] = instance.formElement[prop].replace(
            /\{\{values\.([^\}]+)\}\}/g,
            '{{getValue("$1")}}');
        }
        else {
          ...
          instance[prop] = applyArrayPath(instance.formElement[prop], instance.arrayPath);
        }
        if (instance[prop]) {
          instance[prop] = _.template(instance[prop], valueTemplateSettings)(formData);
        }
      }
      else {
        instance[prop] = instance.formElement[prop];
      }
    });
  ...
  if (this.formElement) {
    ...
    if (this.el) {
      ...
      if (handlers) {
        _.each(handlers, function (handler, onevent) {
          if (onevent !== 'insert') {
            $(instance.el).bind(onevent, function(evt) { handler(evt, node); });
          }
        });
      }
    }
...
formTree.prototype.buildTree = function () {
  ...
  var instance = this;
  _.each(instance.formDesc.form, function (formElement) {
    console.log(formElement, instance);
    if (formElement === '*') {
      _.each(instance.formDesc.schema.properties, function (element, key) {
        instance.root.appendChild(instance.buildFromLayout({
          key: key
        }));
      });
    }
      ...
      instance.root.appendChild(instance.buildFromLayout(formElement));
    }
  });
};
...
formTree.prototype.buildFromLayout = function (formElement, context) {
  ...
  if (formElement.key) {
    ...
    if (schemaElement.type === 'object') {
      _.each(schemaElement.properties, function (prop, propName) {
        node.appendChild(instance.buildFromLayout({
  ...
  if (node.view.array) {
    ...
    _.each(formElement.items, function (item) {
      ...
      node.appendChild(instance.buildFromLayout(item));
    });
  }
